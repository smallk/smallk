...
from pysmallk import matrixgen as mg
...
if args.type == 'UNIFORM':
    A = mg.uniform(args.height, args.width, center=args.rng_center, radius=args.rng_radius)

elif args.type == 'DENSE_DIAG':
    A = mg.densediag(args.height, args.width, center=args.rng_center, radius=args.rng_radius)

elif args.type == 'SPARSE_DIAG':
    A = mg.sparsediag(args.width, center=args.rng_center, radius=args.rng_radius)

elif args.type == 'IDENTITY':
    A = mg.identity(args.height, args.width)

elif args.type == 'ONES':
    A = mg.ones(args.height, args.width)

elif args.type == 'ZEROS':
    A = mg.zeros(args.height, args.width)

elif args.type == 'SPARSE':
    A = mg.sparse(args.height, args.width, args.nz_per_col)
...
mg.write_output(args.filename, precision=args.precision)
