...
more_sources = [smallk_path + "preprocessor/src/main.cpp",
                smallk_path + "preprocessor/src/command_line.cpp",
                smallk_path + "preprocessor/src/preprocess.cpp",
...
if 'macosx' in thisPlatform:
    include = [smallk_path + "common/include/", smallk_path + "preprocessor/include", numpy.get_include()]
    library = []
elif 'linux' in thisPlatform:
    include = ["/usr/local/include", smallk_path + "common/include/", smallk_path + "preprocessor/include", numpy.get_include()]
...
ext_modules = [
    Extension(
        name = "libpreprocess",
        sources = ["interface/preprocess_lib.pyx"] + more_sources,
        include_dirs = include,
        libraries = ["m"],
        library_dirs = library,
        language="c++",
        extra_compile_args=["-std=c++11", "-D ELEM_VER=84"]
    )
    ]
